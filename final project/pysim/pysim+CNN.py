...
def simulator(p):
   ...
   # Parameters for lucas kanade optical flow
    ...
    #import CNN model
    modelWeights = "model.pb"
    net = cv2.dnn.readNetFromTensorflow(modelWeights)
    ...
    for t in range(1, p['sim_len']):
        ...
        sample_im = sample_im.astype(np.float32)
        frame = preprocessing(sample_im)
        frame = cv2.resize(frame,(496,496),interpolation = cv2.INTER_NEAREST)
        blob = cv2.dnn.blobFromImage(frame, swapRB=True, crop=False)
        net.setInput(blob)
        peaks = net.forward()
        peaks[peaks<0]=0
        peaks = peaks.reshape(frame.shape[1], frame.shape[1])
        peaks = cv2.resize(peaks,(500,500),interpolation = cv2.INTER_NEAREST)
        ...
        if t >= p['phase1_len']:
            ...
            if last_im.max() > 0:
                ...
                for i,(new,old) in enumerate(zip(good_new, good_old)):
                    a,b = new.ravel().astype(int).clip(0, p['FOVx']-1)
                    c,d = old.ravel().astype(int).clip(0, p['FOVx']-1)
