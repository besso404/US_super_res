...
up_lim2 = floor((Z2-(D/2))*ppm);
down_lim2 = ceil((Z2+(D/2))*ppm);
dt = 1/FR;                       %[sec/frame]
...
bubbles2 = [struct('y', normrnd(Z2,(D/2)^0.5)*ppm, 'x', FOVx_, ...
    'u', normrnd(mu_u,std_u)*ppm*dt,...
    'v', normrnd(0,std_v)*ppm*dt  , 't0', 1)]; 
...
for t = 1:sim_len
    ...
    if (mod(t, 30) == 0)
        bubbles1 = [bubbles1; struct('y', normrnd(Z1,(D/2)^0.5)*ppm,...
    ...
    for b=1:length(bubbles2)
        ...
        bubbles2(b).x = FOVx_ - (t-bubbles2(b).t0)*bubbles2(b).u;
        
        if (bubbles2(b).y < up_lim2 + abs(bubbles2(b).v) || bubbles2(b).y > down_lim2 - abs(bubbles2(b).v))
            [bubbles2(b).u,bubbles2(b).v] = elastic_collision(bubbles2(b).u,bubbles2(b).v);
        end
             
        if (bubbles2(b).x < 1 + bubbles2(b).u)
            exitted_frame2 = b;
        end

        mask(ceil(bubbles2(b).y), max(1,round(bubbles2(b).x))) = 1;
